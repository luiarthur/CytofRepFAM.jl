...
\def\imgdir{../../results/test-sim-6-7-12}
\input{imgtemplate.tex}

% Title Settings
\title{Simulation Study 6.7.12}
\author{Arthur Lui}
...
\begin{figure}[H]
  ...
  \end{center}
  \caption{Post burn-in log likelihoods for each run. Burn in used was 3000.
  The plot shows the log-likelihood evaluations of the 3000 subsequent
  iterations. The experiment in Figure (w) has not yet converged. Also, some of
  log likelihoods are higher for some of the chains (e.g. (e), (h), (i), (k)). These are
  also the chains that recovered the simulation truth. Experiments (d) and (m) contain
  one selected feature with $W_{i,k}=0$.}
  \label{fig:ll}
\end{figure}
...
\begin{figure}[H]
  ...
  Three to four columns only are recovered. In the case of Figure (a), the
  recovered features were features 3, 5, 6, and 7. Even though feature 4 is
  overall more abundant than feature 7, feature 4 is recovered because it is
  distinct. It appears feature 4 was merged with feature 5, which is very
  similar. When only 3 features are recovered, the missing feature is feature
  7, which makes up only 3\% in sample 1. The truth is sometimes recovered for
  $\phi=1$, and $\phi=10$.}
  \label{fig:zmean}
\end{figure}
