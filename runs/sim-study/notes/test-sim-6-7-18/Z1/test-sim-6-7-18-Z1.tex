...
\begin{itemize}
  \item Features (columns) 1, 2, and 3 are similar (maximum difference of 2).
    ...
    know if the first two features are prone to being absorbed into feature 3.
    We found that using the prior $w^\star_{i,k} \sim \Gam(10, 1)$, and using
    priors that encouraged $r_{i,k}$ to be sparse, Features 1 and 2 tend to
    merge (into Feature 2, which is slightly larger), and Feature 3 would be in
    its own cluster or split into two smaller identical features. We believe
    this is due to prior information in $w^\star$ encouraging features to be
    equally weighted.
  ...
  \item Features 4 and 5 are similar (difference of 1). They are rare / absent
    ...
    these two features will be merged. Although, we didn't expect them to. It
    was conceivable that they will only be ``selected'' in one sample, but not
    in another. We found that they were always recovered, in part, we believe,
    because they had a large presence in one sample. We continue to monitor
    their behaviour here.
  \item Feature 6 is abundant in Sample 1 but not in Sample 2. We expect it to
    be recovered in $Z$ but it is conceivable that it will not be ``selected''
    in sample 2. It is distinct from other features.
...
\begin{figure}[H]
  \begin{center}  % 6 x 5
    \includegraphics[scale=0.5]{\imgdir/dataseed1-mcmcseed1-phi1.0\zind/img/Z_true.pdf}
  \end{center}
  \caption{$Z^\true$}
  \label{fig:Z-true}
\end{figure}
...
\begin{itemize}
  ...
  \begin{itemize}
    ...
    % \item $\eta_{z, i,j,\ell} \sim \Dir_{L_z}(1)$ % Really this.
    % \item $\sigma^2_i \sim \InvGamma(3, 1)$ % Really this.
...
\begin{figure}[H]
  \begin{center}  % 6 x 5
    \imgtemplate{loglike_postburn.pdf}{.15}{\zind}
  \end{center}
  \caption{Log-likelihoods for each simulation setup. Burn-in may be
  insufficient from trace plot in (d).}
  \label{fig:ll}
\end{figure}
...
\begin{figure}[H]
  \begin{center}  % 6 x 5
    \imgtemplate{Zmean.pdf}{0.15}{\zind}
  \end{center}
  % TODO: Write more?
  \caption{With the exception of (c), none of the runs recover 7 columns, so
  none of the runs capture the simulation truth completely. The two features
  that are usually missing are either feature 1 or feature 2, or both. This is
  reasonable, because these features are less abundant, and are similar to
  other more abundant features. With only one component for $\mu_{z,\ell}$ in
  the simulation truth, the posterior mean of $\sigma^2$ is closer to the
  truth, but since $\sigma^2$ is now larger, it is harder to recover the truth
  for $Z$.}
  \label{fig:zmean}
\end{figure}
...
\begin{figure}[H]
  \begin{center}  % 6 x 5
    \imgtemplate{p1_trace.pdf}{0.15}{\zind}
  \end{center}
  % TODO: Write more?
  \caption{}
  \label{fig:p1trace}
...
\begin{figure}[H]
  \begin{center}  % 6 x 5
    \imgtemplate{p2_trace.pdf}{0.15}{\zind}
  \end{center}
  % TODO: Write more?
  \caption{}
  \label{fig:p2trace}
...
\begin{figure}[H]
  \begin{center}  % 6 x 5
    \imgtemplate{mus_trace.pdf}{0.15}{\zind}
  \end{center}
  % TODO: Write more?
...
\begin{figure}[H]
  \begin{center}  % 6 x 5
    \imgtemplate{sig2_trace.pdf}{0.15}{\zind}
  \end{center}
  % TODO: Write more?
  \caption{}
...
\begin{figure}[H]
  \begin{center}  % 6 x 5
    \imgtemplate{sig2.pdf}{0.15}{\zind}
  \end{center}
  % TODO: Write more?
  \caption{}
  \label{fig:sig2}
...
\begin{figure}[H]
  \begin{center}  % 6 x 5
    \imgtemplate{dden/dden_i1_j1.pdf}{0.15}{\zind}
  \end{center}
  % TODO: Write more?
  \caption{}
  \label{fig:ddi1j1}
...
\begin{figure}[H]
  \begin{center}  % 6 x 5
    \imgtemplate{dden/dden_i1_j2.pdf}{0.15}{\zind}
  \end{center}
  % TODO: Write more?
  \caption{}
  \label{fig:ddi1j2}
...
\begin{figure}[H]
  \begin{center}  % 6 x 5
    \imgtemplate{dden/dden_i2_j1.pdf}{0.15}{\zind}
  \end{center}
  % TODO: Write more?
  \caption{}
  \label{fig:ddi2j1}
...
\begin{figure}[H]
  \begin{center}  % 6 x 5
    \imgtemplate{dden/dden_i2_j2.pdf}{0.15}{\zind}
  \end{center}
  % TODO: Write more?
  \caption{}
  \label{fig:ddi2j2}
