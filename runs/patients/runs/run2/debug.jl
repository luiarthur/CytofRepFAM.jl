...
path_to_data = ["$(data_dir)/001_d31_clean.csv",
                "$(data_dir)/007_d35_clean.csv",
                "$(data_dir)/010_d35_clean.csv"]
...
# Print pid's for each loaded processor.
pmap(_ -> println("pid: $(getpid())"), 1:length(workers()))

@time run(phi, path_to_data, results_dir, aws_bucket, K=20,
